...
method that creates "generic" views. Views created this way are generally not
particularly useful as-is. We will usually create our own factory functions 
dedicated to embelishing views created by PiewPiew.View() with useful 
functionality to control the display and handle user interactions.

As an example, let's create a simple view factory method.
...
Here we are simply creating and returning a PiewPiew.View with some default
property values specified. In this example we have specified
...
* `classes:` This is an array of extra css classes to add to the root DOM element
  of the view

Now, create a new ExampleView by calling our custom view factory function
...
a `.templateUrl` property rather than the `.template` previously we just 
looked at. Lets move our Hello World example to an external template. Add the
following to a new file named "ExampleViewTemplate.html"
